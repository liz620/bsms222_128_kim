...
We start by defining the random variable $X$ as: $X=1$ if we pick a blue bead at random and $X=0$ if it is red. This implies that population is a list of 0s and 1s. If we sample $N$ beads, then the average of the draws $X_{1},...,X_{N}$ is equivalent to the proportion of blue beads in our sample. This is because adding the $X$s is equivalent to counting the blue beads and dividing this count by the total $N$ is equivalent to computing a proportion. We use the symbol $\bar{X}$ to represent this average. In general, in statistics textbooks a bar on top of a symbol means the average. The theory we just learned about the sum of draws becomes useful because the average is a sum of draws multiplied by the constant $1/N$:
...
Using what we have learned, the expected value of the sum $N$ $\bar{X}$ is $N×$ the average of the urn, $p$. So dividing by the non-random constant $N$ gives us that the expected value of the average $\bar{X}$ is $p$. We can write it using our mathematical notation:
...
4. What is the stadard error of $\bar{X}$? Hint: it's a function of $p$.
...
10. Given the answer to 9, which is the following best describes your strategy of using a sample size of $N=25$?
...
$Pr(\bar{X} \le p + .01) - Pr(\bar{X} \le p-.01)$
...
$Pr(Z \le \frac{.01}{SE(\bar{X})}) - Pr(Z \le -\frac{.01}{SE(\bar{X})})$
...
$\mbox{Pr}\left(Z \leq \, 1.96\,\mbox{SE}(\bar{X})  / \mbox{SE}(\bar{X}) \right) - \mbox{Pr}\left(Z \leq - 1.96\, \mbox{SE}(\bar{X}) / \mbox{SE}(\bar{X}) \right)$
...
The competition is to predict the spread, not the proportion $p$. However, because we are assuming there are only two parties, we know that the spread is $p-(1-p)=2p-1$. Once we have our estimate $\bar{X}$ and $\hat{SE}(\bar{X})$, we estimate the spread with $2\bar{X}-1$ and, since we are multipying by 2, the standard error is $2\hat{SE}(\bar{X})$. Note that subtracting 1 does not add any variability so it does not affect the standard error.
...
plot %>% ggplot(aes(N,se)) + geom_point() + geom_hline(yintercept = 0.01, linetype = 'dashed', color='blue')
```
...
N <- 100
barX <- 0.51
se_barX <- sqrt(barX*(1-barX)/N)
1 - pnorm(0.01, 0, se_barX) + pnorm(-0.01, 0, se_barX)
