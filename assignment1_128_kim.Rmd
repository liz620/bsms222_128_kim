...
sample size of 'Non-patient' is so so big that it is difficult to compare. Also, the numbers for variant classes other than SNVs of 'Non-patient' are too small to be shown in the plot above. Therefore, as we checked that most of variants in 'Non-patient' group are SNVs, let's compare the variant classes among different diseases.
...
SCN2A_1 %>%
  ...
  group_by(Group) %>%
  count(VARIANT_CLASS) %>%
  ggplot(aes(Group, n, fill = VARIANT_CLASS)) +
  geom_bar(position = "stack", stat = "identity") +
  coord_flip() +
  geom_text(aes(label = n), position = "stack", size = 3.5) +
