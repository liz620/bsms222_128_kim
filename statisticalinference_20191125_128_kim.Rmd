...
2. Now use dplyr to adda confidence interval as two columns, call them lower and upper, to the object poll. Then use select to show the pollster, enddate, x_hat, lower, upper variables. Hint: define temporary columns x_hat and se_hat.

```{r}
polls %>% mutate(x_hat=polls$rawpoll_clinton[1]/100, se_hat = sqrt(x_hat*(1-x_hat)/samplesize), lower =x_hat -qnorm(0.975)*se_hat, upper =  x_hat +qnorm(0.975)*se_hat) %>%
...
```{r}
polls_1 <- polls %>% mutate(x_hat=polls$rawpoll_clinton[1]/100, se_hat = sqrt(x_hat*(1-x_hat)/samplesize), lower =x_hat -qnorm(0.975)*se_hat, upper =  x_hat +qnorm(0.975)*se_hat, hit = lower <= 0.482 & upper >= 0.482) %>%
...
```{r}
polls_2 <- polls %>% mutate(x_hat= (d_hat+1)/2, se_hat = 2*sqrt(x_hat*(1-x_hat)/samplesize), lower =d_hat - qnorm(0.975)*se_hat, upper =  d_hat + qnorm(0.975)*se_hat, hit = lower <= 0.021 & upper >= 0.021) %>%
...
polls %>% mutate(error = d_hat - 0.021) %>%
  ...
  geom_point() + theme(axis.text.x = element_text(angle = 30, hjust = 1))
```
...
The story is as follows: an acquaintance of Fisher's claimed that she could tell if milk was added before or after tea was poured. Fisher was skeptical. He designed an experiment to tst this claim. He gave her four pairs of cups of tea: one with milk poured first, the other after. The order was randomized. The null hypothesis here is that she is guessing. Fisher derived the distribution for the number of correct picks on the assumption that the choices were random and independent.
...
se <- two_by_two %>% select(-awarded) %>% summarize(se = sqrt(sum(1/men) + sum(1/women))) %>%
  pull(se)
...
```{r}
two_by_two <- data.frame(result = c("win", "loss"), important = c(8, 9), less_important = c(350-8, 150-9))

chisq_test <- two_by_two %>% select(-result) %>% chisq.test()
...
odds_less_important <- with(two_by_two, (less_important[1]/sum(less_important)) / (less_important[2]/sum(less_important)))

log_or <- log(odds_important / odds_less_important)
se <- two_by_two %>% select(-result) %>%
  summarize(se = sqrt(sum(1/important) + sum(1/less_important))) %>%
  pull(se)
ci <- log_or + c(-1,1) * qnorm(0.975)*se
exp(ci)
```
...
```{r}
two_by_two %>% select(-result) %>%
  mutate(important = important*2, less_important = less_important*2) %>%
  chisq.test() %>% .$p.value
```
