...
We can drop the x = and y = if we want to since these are the first and second expected argument, as seen in the help page.
...
an we want to make a scatterplot with ggplot, we would have to type something like:
...
4. What is the class of the object p you ahve just created?
...
8. If instead of points we want to add text, we can use the geom_text() or geom_label() geometries. The following code will give us the error message.
...
10. Change the color of the labels through blue. How will do this?
