...
```{r}
## I have started a new project, so i will add a rate column again.
...
With arrange we get to decide which column to sort by. To see the states by rate, from smallest to largest, we arrange by rate instead:
...
Note that rows are not sorted by rate, only filtered. If want to sort, we need to use arrage. Note that if the third argument is left blank, top_n, filters by the last column.
...
2. Using a pipe, assign the average to a numeric variable ref_avg. Hint: Use the code similar above and then pull.
...
```{r}
murders_t <- as_tibble(murders)
## checked on concole
## 'A tibble: 51 x 5' shown / all the five columns showed on the same line.
```

### 5.10.2 Subsets of tibbles are tibbles

If you subset the columns of a data frame, you may get back an object that is not a dat frame, such as a vector or scalar. For exmaple:
...
## why? with the function summarize, we can only call functions that return a single value(5.7.1)
```
...
If you do not use the parenthesis, then the function is not executed and instead do tries to return the function. This gives an error because do must always return a dat aframe. You can see the error by typing:
...
## the smaller number of the range should be the second argument.
```
...
=> it is tidy and its class is data.frame.

**Hadley Wickham later defined "Tidy Data" as data sets that are arranged such that each variable is a column and each observation (or case) is a row.**
...
```{r}
abc <- murders %>% group_by(region)
class(abc)
## checking class if it has changed to a tibble
```
...
data_s_n <- map_df(n, compute_s_n)
n <= 100
data_s_n %>% mutate(n = n, s_n, s_n_2 = s_n)

## what's wrong with the order of the columns.....
## maybe i should define those column at first when i set the data frame.
## and why do i have all those TRUE signs... something wrong with n?
```
...
yay!
*i don't need to mutate the data frame again here because it's not the given data frame that i'm using. i can just define all the columns i want at first.

*if you write '<', '<=', etc, R thinks that i am asking if it is true or not. When you want to define something, please use '<-'
