...
We use the notation $Pr(A)$ to denote the probability of event $A$ happening. We use the very general term *event* to refer to things that can happen when something occurs by chance. In our previous example, the event was "picking a read bead". In a political poll in which we call 100 likely voters at random, an example of an event is "calling 48 Democrats and 52 Republicans".

In data science applications, we will often deal with continuous variables. These events will often be things like "is this person taller than 6 feet". In this case, we write events in a more mathematical form: $X \ge 6$. We will see more of these examples later. Here we focus on categorical data.
...
If you have to guess the color of the first bead, you will predict blue since has a 60% chance. But if I show you the result of the last four outcomes:
...
$Pr(cyanball) = \frac{3}{3+5+7} = \frac{1}{5}$
...
$Pr(notcyanball) = 1 - Pr(cyanball) = \frac{4}{5}$
...
$Pr() = \frac{3}{15} × \frac{12}{14} = \frac{6}{35}$
...
$Pr() = \frac{3}{15} × \frac{12}{15} = \frac{4}{25}$
...
It is done with replacement, so the probability doesn't change.

$Pr(yellow) = \frac{5}{15} = \frac{1}{3}$
...
'Not seeing a 6' means that a side for 6 is always at the bottom so I cannot read it. The probability for the side of 6 to be at the bottom is $\frac{1}{6}$

$Pr() = (\frac{1}{6})^{6} = \frac{1}{6^{6}}$
...
cavs_winning_series <- rowSums(p) >= 4
mean(cavs_winning)
```
...
```{r}
B <- 10000
cavs_winning_series <- replicate(B, {
  cavs_wins <- sample(c(0,1), 6, replace = TRUE)
  sum(cavs_wins) >= 4
})
mean(cavs_winning_series)
```
