...
![Source: Fox News, via Media Matters](https://rafalab.github.io/dsbook/dataviz/img/class2_8.jpg)
...
![Source: Fox News, via Flowing Data](https://rafalab.github.io/dsbook/dataviz/img/Bush-cuts.png)
...
![Source: Pakistan Today via Diego Mariano](https://rafalab.github.io/dsbook/dataviz/img/venezuela-election.png)
...
![Source: The 2011 State of the Union Address: Enhanced Version via Peter Aldhous](https://rafalab.github.io/dsbook/dataviz/img/class2_30.jpg)
...
To motivate our first principle, 'show the data', we go back to our artificial example of describing heights to ET, an etraterrestrial. This time let's assume ET is interested in the difference in heights between males and females. A commonly seen plot used for comparisons between groups, popularized by software such as Microsoft Excel, is the dynamite plot, which shows the average and standard errors (standard errors are defined in a later chapter, but do not confuse them with the standard deviation of the data). The plot looks like this:

![](https://rafalab.github.io/dsbook/book_files/figure-html/show-data-1-1.png)

The average of each group is represented by the top of each bar and the antennae extend out from the average to the average plus two standard errors. If all ET receives is this plot, he will have little information on what to expect if he meets a group of human males and females. The bars go to 0: does this mean there are tiny humans measuring less than one foot? Are all males taller then the tallest females? Is there a range of heights? ET can't answer these questions since we have provied almost no information on the height distribution.
...
Using a log transformation here provides a much more informative plot. We compare the original barplot using the log scale transformation for the y-axis:
...
dat %>%
  mutate(location = ifelse(year == 2010, 1, 2), location = ifelse(year == 2015 & country %in% c("United Kingdom", "Portugal"), location + 0.22, location),
...
![Source: Karl Broman](https://rafalab.github.io/dsbook/dataviz/img/fig8b.png)
...
![Source: Karl Broman](https://rafalab.github.io/dsbook/dataviz/img/fig1e.png)

![Source: Karl Broman](https://rafalab.github.io/dsbook/dataviz/img/fig2d.png)
...
Answer: A. When we want to display percentages.
...
Answer: B. The axis does not start at 0. Judging by the length, it appears Trump received 3 times as many votes when, in fact, it was about 30% more.
...
Answer: C. The plot on the right is better because alphabetical order has nothing to do with the disease and by ordering according to actual rate, we quickly see the states with most and least rates.
...
Answer: C. It does not show all the data. We do not see the variability within a region and it’s possible that the safest states are not in the West.
...
Answer: A. Humans are not good at reading pseudo-3D plots.
...
10, 11, 12 are in the **11.15 Exercises**
...
```{r}
the_disease_1<- "Smallpox"

dat_1 <- us_contagious_diseases %>%
  filter(disease == the_disease_1 & !weeks_reporting < 10 ) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  mutate(state = reorder(state, rate))
```
...
```{r}
dat_1 %>% ggplot(aes(year, state, fill = rate)) +
  geom_tile(color = "grey50") +
  scale_x_continuous(expand = c(0,0)) +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Reds"), trans = "sqrt") +
  geom_vline(xintercept = 1963, col = "blue") +
  theme_minimal() + theme(panel.grid = element_blank()) +
  ggtitle(the_disease_1) +
  ylab("") +
  xlab("")
```
...
```{r}
avg_1 <- us_contagious_diseases %>%
  filter(disease == the_disease_1) %>% group_by(year) %>%
  summarize(us_rate_1 = sum(count, na.rm=TRUE)/sum(population, na.rm=TRUE)*10000)
```
...
dat_1 %>%
  ...
  ggplot() +
  geom_line(aes(year, rate, group = state), color = "grey50", show.legend = FALSE, alpha = 0.2, size =1) +
  geom_line(mapping = aes(year, us_rate_1), data = avg_1, size = 1, color = "black") +
  scale_y_continuous(trans = "sqrt", breaks = c(5, 25, 125, 300)) +
  ggtitle("Cases per 10,000 by state") +
  xlab("") +
  ylab("") +
  geom_text(data = data.frame(x = 1933, y = 4), mapping = aes(x, y, label = "US average"), color = "black") +
...
```{r}
dat_Cal <- us_contagious_diseases %>%
...
dat_Cal %>%
  filter(!is.na(rate)) %>%
  ggplot() +
  geom_line(aes(year, rate, group = state, col = disease), show.legend = FALSE, alpha = 0.5, size =1) +
  scale_y_continuous(trans = "sqrt", breaks = c(5, 25, 125, 300)) +
  ggtitle("Cases in California") +
  xlab("") +
  ylab("") +
  geom_vline(xintercept = 1970, col = "blue")
```
...
```{r}
total_avg <- us_contagious_diseases %>%
  group_by(year) %>%
  summarize(us_total_rate = sum(count, na.rm=TRUE)/sum(population, na.rm=TRUE)*10000)
```

```{r}
total_avg %>%
  ggplot(aes(year, us_total_rate)) +
  geom_line(color = "black", size = 1) +
  scale_y_continuous(trans = "sqrt", breaks = c(5, 25, 125, 300)) +
  ggtitle("Rates for the US") +
  xlab("") +
  ylab("") +
  geom_vline(xintercept = 1970, col = "blue")
```
