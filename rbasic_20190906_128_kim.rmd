...
### 3.2.1 Objects
Suppose a high school student asks us for help solving several quadratic equations of the form $ax^{2}+bx+c=0$. The quadratic formula gives us the solution:

$\frac{-b-\sqrt{b^{2}-4ac}}{2a}$ and $\frac{-b+\sqrt{b^{2}-4ac}}{2a}$ 

which of course change depending on the values of a, b and c. One advantage of programming languages is that we can define variables, similar to how we do so in math, but obtain a numeric solution. We will write out general code for the quadratic equation below, but if we are asked to solve $x^{2}+x-1=0$, then we define:
...
1. What is the sum of the first 100 positive integers? The formula for the sum of integers 1 through n is $n(n+l)/2$. Define $n=100$ using the formula. What is the sum?
...
5. We saw that the region columnstores a factor. You can corroborate this by typing:
