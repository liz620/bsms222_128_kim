...
We will be working with two types of variables: categorical and numeric. Each can be divided into two other groups: categorical can be ordinal(서수, 순서를 나타냄) or not, whereas numerical variables can be discrete or continuous.
...
A. The graph above is a histogram. -> This is a plot for distribution.

C. Categories are not numbers, so it does not make sense to graph the distribution. -> THIS doesn't make sense. You can graph the distribution with categories.

D. The colors, not the height of the bars, describe the distribution.-> The colors of the bars are just describing different categories. The height of the bars describe the distribution.
...
**Answer: D. They are the same dataset, but the first is not in the log scale, the second undersmooths and the third oversmooths.**
...
You don’t need to memorize or understand the details of the formula. But note that it is completely defined by just two parameters: m and s. The rest of the symbols in the formula represent the interval ends that we determine,  
...
1. Define a vector of m proportions $p_{1}, p_{2},..., p_{m}$.
2. Define a vector of quantiles $q_{1},..., q_{m}$ for your data for the proportions $p_{1},...,p_{m}$. We refer to these as the sample quantiles.
3. Define a vector of theoretical quantiles for the proportions $p_{1},...,p_{m}$ for a normal distribution with the same average and standard deviation as the data.
...
```{r}
approximate <- pnorm(81, avg, sd) - pnorm(79, avg, sd)
...
```{r}
seven_footers_total <- seven_footers*10^9

10 / seven_footers_total
```
...
```{r}
Lebron_James <- 1 - pnorm(6*12+8, 69, 3)
Lebron_James
Lebron_James_total <- Lebron_James*10^9
Lebron_James_total

150 / Lebron_James_total
```
...
**Answer: C. As seen in question 3, the normal approximation tends to underestimate the extreme values. It’s possible that there are more seven footers than we predicted.**
