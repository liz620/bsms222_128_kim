...
daticipants. Basically, we construct an estimate of the spread, let's call it $d$, with a weighted average in the following way:
...
3. What does the theory tell us about sample average $\bar{X}$ and how it is related to $\mu$?
...
```{r}
s <- sd(X)
mean(X) + c(-1,1)*qnorm(1-0.05/2)*s/sqrt(N)
```
...
result <- replicate(B, {
  ...
  interval <- mean(X) + c(-1,1)*qnorm(1-0.05/2)*s/sqrt(N)
...
with $i=1,2$ indexing the two pollster, $b_{i}$ the bias for pollster $i$ and $\epsilon_{i,j}$ poll to poll chance variability. We assume the $\epsilon$ are independent from each other, have expected value 0 and standard deviation $\sigma_{i}$ regardless of $j$.
...
${E}[Y_{1,j}] = \mbox{E}[d + b_1 + \varepsilon_{i,j}] = d+b_1+\mbox{E}[\varepsilon_{i,j}] = d + b_1$
...
${SE}[\bar{Y}_{2} - \bar{Y}_1] = \sqrt{\mbox{SE}[\bar{Y}_{2}]^2 + \mbox{SE}[\bar{Y}_1]^2} = \sqrt{\sigma_2^2/N_2 + \sigma_1^2/N_1}$
...
```{r}
polls %>% group_by(pollster) %>% summarize(s = sd(spread))
```
...
estimate <- result$avg[2] - result$avg[1]
se_hat <- with(result, sqrt(s[2]^2/N[2] + s[1]^2/N[1]))
...
```{r}
polls %>% group_by(pollster) %>% summarize(avg = mean(spread), sd = sd(spread))
```
