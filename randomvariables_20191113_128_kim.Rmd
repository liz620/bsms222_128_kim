...
$\begin{aligned}
\mbox{SE}[(X_1+X_2+\dots+X_n) / n] &=
\mbox{SE}[X_1+X_2+\dots+X_n]/n \\
&= \sqrt{\mbox{SE}[X_1]^2+\mbox{SE}[X_2]
2+\dots+\mbox{SE}[X_n]^2}/n \\
...
Not that statistical textbooks use the Greek letters $\mu$ and $\sigma$ to denote the expected value and standard error, respectively. This is because $\mu$ is the Greek letter for $m$, the first letter of *mean*, which is another term used for expected value. Similarly, $\sigma$ is the Greek letter for $s$, the first letter of standard error.
...
```{r}
x <- sample(c(17, -1), prob = c(1/19, 18/19))
```
...
loss_per_foreclosure <- -200000
default_rate <- 0.3
loans <- sample(c(0,1), n, replace=TRUE, prob=c(1-default_rate, default_rate))
...
```{r}
n*(default_rate*loss_per_foreclosure + (1- default_rate)*0)
```

4. What is the standard error of $S$?

```{r}
sqrt(n)*abs(loss_per_foreclosure)*sqrt(default_rate*(1-default_rate))
```

5. Suppose we give out loans for $180,000. What should the interest rate be so that our expected value is 0?

```{r}
x <- -(loss_per_foreclosure*default_rate) / (1 - default_rate)
x / 180000
```
...
```{r}
z <- qnorm(0.05)
x <- -loss_per_foreclosure*(n*default_rate - z*sqrt(n*default_rate*(1-default_rate))) / (n*(1-default_rate)+z*sqrt(n*default_rate*(1-default_rate)))
x / 180000
```
