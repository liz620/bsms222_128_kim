...
Answer: D. co2 is not tidy: to be tidy we would have to wrangle it to have three columns (year, month and value), then each co2 observation would have a row.

A: one year has two rows of data
...
```{r}
# Time-Series Objects가 뭐지
# > 'These are vectors or matrices with class of "ts" (and additional attributes) which represent data which has been sampled at equispaced points in time.'
# C,D 둘다 답인가????
```
...
A: it has one row for each chick

C: do we need an year column?

D: not all data in a data frame is tidy
...
A: nothing to do with numbers of rows.

B: 이게 안 될 이유가..?

D: i dont think it's related to how tidy it is.
...
DNase
```
-> column for run doesn't look tidy for me. Too many same values.
...
Orange
```
-> The column for tree doesn't look tidy for me. Too many trees with same number.
...
Use the function mutate to add a murders column named rate with per 100,000 murder rate as in the example code above. Make sure you redefine murders as done in the example code above (murders <- [your code]) so we can keep using this variable.
...
new_table <- select(my_states, state, rate, rank)
new_table
```
