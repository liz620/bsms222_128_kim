...
6. Repeat the previous exercise, but this time order my_df so that the states are ordered from least populous. Hint: create an object ind that stores the indexes needed to order the population values. Then use the bracket operator [ to re-order each column in the data frame.
...
This is.na function returns a logical vector that tells us which entries are NA. Assign this logical vector to an object called ind and determine how many NAs does na_example have.
...
```{r}
temp_cel <- 5*(temp - 32)/9
city_temps <- data.frame(name = city, Fahrenheit_temperature = temp, Celsius_temperature = temp_cel)
```

2. What is the following sum $1 + \frac{1}{2^{2}} + \frac{1}{3^{2}} + ... \frac{1}{100^{2}}$ ? Hint: thanks to Euler, we know it should be close to $\frac{\pi^{2}}{6}$.
...
For a quick plot that avoids accessing varaibles twice, we can use the with function:
